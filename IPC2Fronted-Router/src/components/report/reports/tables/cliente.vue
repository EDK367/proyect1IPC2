  ...
  <div>
    ...
    <div class="table_Container">
      ...
      <v-table height="300px" fixed-header>
        ...
        <tbody>
          <tr v-for="reporte in filteredTrayecto" :key="reporte.NIT">
            <td>{{ reporte.NIT }}</td>
            <td>{{ reporte.total }}</td>
          </tr>
        </tbody>
...
export default {
  ...
  computed: {
    ...
    filteredTrayecto() {
      ...
      return this.reporte.filter((reporte) => {
        return reporte.NIT.toString() === this.searchTerm;
      });
  ...
  methods: {
    obtenerReporte() {
      axios
        .get("http://localhost:8080/backendIPC2/api/reportes?op=cliente")
        .then((response) => {
